...
9. Never merge the pull request, and never merge into `main`. The gate merges `origin/main` into the card branch. Never force-push, or use `--no-verify`, `--admin` or bare `docker compose`.
...
12. Push, open the pull request ready or link the open one, add its changelog fragment, and push. Link it (contract rule 5).
