...
**Running tests in parallel worktrees:** `config/packages/doctrine.yaml` sets `dbname_suffix: '_test%env(default::TEST_TOKEN)%'`, so PHPUnit's database is `app_test<TEST_TOKEN>`. To run `just ci` / PHPUnit in a git worktree without colliding with the main checkout or a sibling worktree, export a unique `TEST_TOKEN` (e.g. `TEST_TOKEN=_wt1`) so each gets its own `app_test_wt1` schema (bootstrap drop→create→migrate is per-DB). `just worktree-up` writes that token for you. Since each worktree also has its own **dev** database (`app_wt_<name>`, selected by `WORKTREE_DB_SUFFIX` in its `.env.local`), `just migrate-run` and `bin/console` in a worktree never touch the main dev database. Mailpit is no longer shared — each worktree gets its own sidecar at `mailpit-<slug>.<project>.dev.localhost`, and `just e2e` exports `MAILPIT_URL` pointing at it — but **`just e2e` is still serialized**: `playwright.config.ts` keeps `workers: 1` because nothing has yet gated two branches concurrently to prove the rest of the suite is parallel-safe. Removing the mail coupling was the blocker; proving the rest is a separate step.

**CI's `e2e` check is the gate; a local run is for debugging a spec.** Do not run the full suite locally before opening a PR. `.github/workflows/ci.yml` runs the same `just e2e` against a disposable stack on an isolated runner, with both `E2E_BASE_URL` and `MAILPIT_URL` set — push, then read the check, and fix every failure it reports including pre-existing ones. Locally the suite is slower, destructive, and less truthful: across one wave of five branches every local e2e failure was environmental (mail misrouting, cross-worktree contention) and CI passed all five. Run it locally for a named spec you are working on (`just e2e tests/<area>/<spec>.spec.ts`), or for a branch you cannot push yet — not to gate.

Everything below is how to run it locally when one of those cases applies.
