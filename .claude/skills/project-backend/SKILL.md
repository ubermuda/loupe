...
- **Injecting a FormView into a Twig component via forward:** When a form controller needs to re-render a page with an invalid form, pass the FormView as a request attribute to `forward()`: `$this->forward(ShowFooController::class, array_merge($params, ['myForm' => $form->createView()]))`. In the receiving controller, retrieve it with `$this->getInjectedFormView($request, 'myForm')` (a helper on `AppController`) and fall back to a fresh form with `??`: `$form = $this->getInjectedFormView($request, 'myForm') ?? $this->createForm(...)->createView();`. Always pair this with a 422 status code (see the project-frontend skill).
