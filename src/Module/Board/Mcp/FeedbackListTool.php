 ...
 *
 * @phpstan-type FeedbackListItem array{id: string, url: string, anchors: list<FeedbackAnchorSummary>, body: string, hasDrawing: bool, status: string, createdAt: string, cardId: ?string, number: ?int, title: ?string}
 */
#[McpTool(name: self::NAME, description: 'List the feedback of the project bound to your MCP token. Feedback is what a reviewer leaves on a page with the site-review widget, and each item belongs to a card. Each item has its id, the page url, the anchors (the elements it points at), the body, hasDrawing, the status, createdAt, and the cardId, number and title of its card. Returns the pending items by default. Pass status to read the addressed or the resolved ones, or all. Fix each item, then mark it with feedback_mark_addressed. To read the feedback of one card, use card_get.')]
