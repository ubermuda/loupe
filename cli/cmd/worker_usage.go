...
// such line has a zero baseline. incomplete says messages came after the line.
func sessionBaseline(sessionID string) (baseline *transcript.Usage, incomplete bool) {
	...
	usage, complete, err := transcript.LastCostState(path)
	if err != nil {
		return nil, false
	}
...
// An incomplete baseline or none makes the result an estimate. A process that
...
func workerUsage(rec runRecord, reported transcript.Usage) *api.Usage {
	...
	case reported != nil && rec.Baseline != nil && rec.BaselineIncomplete:
		return apiUsage(api.UsageEstimated, reported.Minus(*rec.Baseline))
