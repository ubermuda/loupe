...
- `getInjectedFormView(Request $request, string $key): ?FormView` — retrieves a `FormView` forwarded via request attribute; returns `null` when nothing was forwarded (see above).
...
**How to log:**
- In controllers and services alike: inject `LoggerInterface` via constructor and call `$this->logger->info('event.name', ['key' => $value])` (there is no `getLogger()` helper on `AppController`)
