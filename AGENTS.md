...
- `main` is protected and takes `--squash` only. **Put the reasoning in the commit messages, not only in the PR body.** GitHub's squash default concatenates the branch's commit messages, so what survives in `git log` is what the commits said; the body survives only on the pull request page unless you pass `gh pr merge <n> --squash --body-file <file>`. A branch with substantive commit messages loses only the body's framing. A branch with thin ones loses everything. It requires one approving review and twelve CI checks. On 2026-09-20 the ruleset named `lint`, `cs-check`, `phpstan`, `arkitect`, `gamache`, `audit`, `phpunit`, `e2e`, `js-test`, `cli-test`, `e2e-chromium` and `e2e-rest`. `e2e` registers last, so a reading that counts eleven checks looks settled and is not. Read that list from the ruleset rather than from here, because nothing in the repository fails when it goes stale. `working-with-prs` carries the command. An approval in chat is not a GitHub approval.
