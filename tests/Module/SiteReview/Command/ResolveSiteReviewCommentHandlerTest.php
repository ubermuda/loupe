...
final class ResolveSiteReviewCommentHandlerTest extends KernelTestCase
{
    private EntityManagerInterface $em;
    private ResolveSiteReviewCommentHandler $handler;
    ...
    protected function setUp(): void
    {
        ...
        $this->handler = new ResolveSiteReviewCommentHandler($this->em, $this->audit->auditor);
    }

    public function test_marks_the_comment_resolved(): void
    {
        $comment = $this->comment('resolve-a@example.com');
    ...
    {
        $comment = $this->comment('resolve-audit@example.com');

        ($this->handler)(new ResolveSiteReviewCommentCommand($comment));

        $record = $this->audit->record('site_review.comment.resolved');
        ...
        self::assertSame(['site_review.comment.resolved'], $this->audit->domainLogLines());
    ...
    private function comment(string $email): SiteReviewComment
    {
        ...
        $project = new Project($user, 'resolve-site');
