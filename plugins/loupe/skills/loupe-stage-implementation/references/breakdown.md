...
2. Read every page of `card_list` with `parentCardId` set to the card id and `full` set, so each row carries its body. Match each entry of the Breakdown section to a child, by the first line, then by the exact title. Leave a child that matches no entry as it is. A child that matches by title only has no first line. Add the first line to the top of its body with `card_update`, and send the rest of the body unchanged.
