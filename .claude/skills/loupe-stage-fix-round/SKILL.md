...
2. Read the failing checks first, then the review threads and the reviews, as the reference says.
3. When no check fails, no thread needs action, and no review is open, stop with `STAGE RESULT: nothing to fix`.
