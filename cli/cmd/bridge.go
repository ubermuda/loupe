...
func runForeground(cmd *cobra.Command, cfg config.Config, site, target string, h transport.Handler, out io.Writer) error {
	...
	if err := transport.Subscribe(ctx, &http.Client{}, creds.HubURL, creds.Topic, jwtRefresher(cfg, site), h); err != nil && ctx.Err() == nil {
		return err
	}
...
func runAttached(cmd *cobra.Command, cfg config.Config, site, target string, h transport.Handler, logPath string) error {
	...
	go func() {
		_ = transport.Subscribe(ctx, &http.Client{}, creds.HubURL, creds.Topic, jwtRefresher(cfg, site), h)
	}()
...
func jwtRefresher(cfg config.Config, site string) transport.TokenFunc {
