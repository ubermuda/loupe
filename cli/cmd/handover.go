...
// lock file and of the control socket, and OldVersion and OldBinary the image
// that froze it, for a rollback.
...
// own rule set by rule name, as a reload does, to rebuild the prompt.
...
// drain waits until no run report waits to go out, no ask check runs and no
// worker is between its spawn and its start. A zero timeout is drainTimeout.
...
func (r *router) drain(ctx context.Context, timeout time.Duration) error {
	...
	defer tick.Stop()
	for {
		reports, checks, starting := r.inFlight()
		if reports == 0 && checks == 0 && starting == 0 {
			return nil
		}
		select {
		case <-ctx.Done():
			return ctx.Err()
		case <-deadline.C:
			return fmt.Errorf("after %s the bridge still holds %d run reports, %d ask checks and %d starting workers", timeout, reports, checks, starting)
		case <-tick.C:
		}
	}
}

func (r *router) inFlight() (reports, checks, starting int) {
	r.mu.Lock()
	checks, starting = r.checking, r.active-len(r.live)
	r.mu.Unlock()
	if r.reports != nil {
		reports = r.reports.Pending()
	}

	return reports, checks, starting
}
...
func (r *router) freeze() handoverState {
	...
	for _, p := range r.queue {
		st.Queue = append(st.Queue, handoverPending{
	...
	for _, run := range r.live {
		st.Live = append(st.Live, handoverRun{
			...
			Began: run.began, PID: run.proc.pid, Dir: run.proc.dir,
		})
	}
...
func (r *router) adoptLocked(run handoverRun) {
	p := pending{key: run.Key, rule: run.Rule, event: run.Event, runID: run.RunID}
	...
	go func() {
		...
		r.settle(p, res, run.Began, time.Since(run.Began))
	}()
}
