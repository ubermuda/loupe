...
- `POST /dev/oauth/access-token` (form: `scopes`, optional `project`) gives the signed-in user an access token, with no consent page. `scopes` is a space-separated list of base scopes, such as `agent mcp` or `site-review`. The handler adds the binding: a scope that needs a project takes the one `project` names, or every project of the owner. `accessToken` (fixtures.ts) wraps it, and `agentAccessToken` is the `agent mcp` shorthand.

  Use it in any spec that calls an API and is about something else. Do not use it to test signing in: the three flows that issue a token in production are covered by PHPUnit under `tests/Module/OAuth/`, and `site-review/widget-oauth.spec.ts` walks the widget popup in the browser.
- `GET /dev/site-review-harness?email=<user-email>` renders a page that embeds the widget by project. It finds or creates that user's `e2e-harness` project, clears the project's comments, and adds the page's own origin to the allowed sites. The reviewer signs in through the OAuth pop-up, so there is no token to read and no seeding endpoint. Press **Sign in with Loupe** in the widget, then drive its own UI to make a comment. `site-review/widget-oauth.spec.ts` walks the whole flow. Pass `&keep=1` to skip the purge.
