...
- `main` is protected and takes `--squash` only. The PR body does **not** become the commit body on its own: GitHub's default concatenates the branch's commit messages, so a multi-commit branch lands a bulleted list and its body survives only on the pull request page. Pass the body explicitly with `gh pr merge <n> --squash --body-file <file>` when the reasoning belongs in `git log`. It requires eight CI checks and one approving review. An approval in chat is not a GitHub approval.
