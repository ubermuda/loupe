...
class ResendVerificationEmailController extends AppController
{
    public function __construct(
        ...
        private readonly RateLimiterFactoryInterface $resendVerificationEmailLimiter,

        #[Autowire(service: 'limiter.resend_verification_email_address')]
        private readonly RateLimiterFactoryInterface $resendVerificationEmailAddressLimiter,
    ...
    public function __invoke(Request $request): Response
    {
        ...
        // Reported rather than silent, unlike the sign-up flows: the bucket is
        // consumed for every address, account or not, so a visible throttle
        // reveals nothing the caller did not already supply.
        $addressLimiter = $this->resendVerificationEmailAddressLimiter->create(($this->addressKey)((string) $email));
