...
1. P0: Start. Get the card into the Product design column, with the slug from the profile.
   - With no card, call `card_create` with `status` set to the slug and `reporter` set to `human`. Take the title and the body from the prompt. A create writes no move event, so no bridge rule fires.
...
9. P9: Readback. When no open decision is left, summarise the shared understanding in a few lines. Let the owner confirm it. Write nothing before the owner confirms.
