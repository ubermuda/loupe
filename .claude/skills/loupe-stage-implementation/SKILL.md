...
5. `card_update` replaces the whole `documentIds`, `pullRequestUrls` and `body`. Send the `card_get` values plus your addition.
...
2. Turn a column label from the prompt into a slug: lowercase, spaces as hyphens. When it differs from the card `status`, stop with `STAGE RESULT: card left <column>`.
3. Find the tech design in `card_get` `documents`. It has the tags `design` and `decisions`, or a title that starts `Tech design`. Read the tags with `document_get`. When none is `approved`, stop with `STAGE RESULT: no approved tech design`.
...
6. From the main checkout, provision `.claude/worktrees/card-<number>` on a new `card-<number>-<short-slug>` branch from `origin/main`. Reuse an existing worktree.
...
12. Wait up to 60 minutes for CI on the pushed head. Fix each failure and push. Run the Codex review per `working-with-prs`.
