...
final class DeleteCommentHandlerTest extends KernelTestCase
{
    ...
    {
        $site = $this->site('del-a@example.com');
        $comment = ($this->addHandler)(new AddCommentCommand($site, 'to delete', '', '', 'https://app/x'));
        $id = $comment->id ?? throw new \LogicException('comment id must not be null');

        ($this->handler)(new DeleteCommentCommand($site, $id));
    ...
    {
        $site = $this->site('del-b@example.com');
        ...
        ($this->handler)(new DeleteCommentCommand($site, $comment->id ?? throw new \LogicException('comment id must not be null')));
    }

    public function test_other_sites_comment_throws_comment_not_found(): void
    {
        $siteA = $this->site('del-c@example.com', 'site-a');
        $siteB = $this->site('del-d@example.com', 'site-b');
