...
final class RegenerateProjectMcpTokenHandlerTest extends KernelTestCase
{
    private EntityManagerInterface $em;
    private RegenerateProjectMcpTokenHandler $handler;
    ...
    protected function setUp(): void
    {
        ...
        $this->handler = new RegenerateProjectMcpTokenHandler($this->em, $projects, $this->audit->auditor);
    }

    public function test_a_regenerated_token_is_recorded_on_the_security_channel(): void
    {
        $project = $this->project('regen-mcp-audit@example.com', 'audited');
        $replacedId = $this->mintToken($project);

        $raw = ($this->handler)(new RegenerateProjectMcpTokenCommand($project));
        ...
        $record = $this->audit->record('project.mcp_token_regenerated');
        ...
        self::assertSame(['project.mcp_token_regenerated'], $this->audit->securityLogLines());
    ...
    {
        $project = $this->project('regen-mcp-audit-fresh@example.com', 'fresh');

        ($this->handler)(new RegenerateProjectMcpTokenCommand($project));
    ...
    {
        DirectLogging::assertRemovedFrom(RegenerateProjectMcpTokenHandler::class);
    }
    ...
    {
        $project = $this->project('regen-mcp-rollback@example.com', 'rolled-back');
        $handler = new RegenerateProjectMcpTokenHandler($this->failingCommitEntityManager(), $this->projects, $this->audit->auditor);

        try {
            $handler(new RegenerateProjectMcpTokenCommand($project));
    ...
    {
        [$token] = ApiToken::issue($project->owner, 'MCP: seeded', ApiTokenScope::Mcp);
