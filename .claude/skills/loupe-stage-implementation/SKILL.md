...
2. Slug the prompt's column label (`references/commands.md`). When it differs from the card `status`, stop with `STAGE RESULT: card left <column>`.
...
4. Run `gh pr view <url> --json state,headRefName` per linked URL. An open pull request on a `card-<number>-` branch: take steps 5 to 7, then skip to the gate. For any other open one, stop with `STAGE RESULT: open pull request exists <url>`.
...
8. Invoke `loupe-documents`. Write the plan with `superpowers-extended-cc:writing-plans`, and submit it tagged `plan`, with `references` set to the tech design id. Link it (contract rule 5).
...
12. Wait up to 60 minutes for CI. Fix failures, then gate and push again.
13. When CI and review are clean, stop with `STAGE RESULT: ready <pr url>`.
