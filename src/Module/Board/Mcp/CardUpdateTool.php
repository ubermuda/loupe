 ...
 */
#[McpTool(name: self::NAME, description: 'Change a card on the project board. Every field but the card id is optional, and a field you leave out keeps the value it has. status takes the slug of a column on this board. Moving a card to a finished column such as done stamps its completion time; moving it back to an open column clears that stamp. A change of status or priority appends the card to the end of the column it arrives in. pullRequestUrls and documentIds are the fields where leaving one out and sending an empty list differ: leave one out and those links stay, send an empty list and every link of that kind is removed. A documentId naming no document of this project is refused. Reporter cannot be changed, because it records who first raised the card. To finish a card, move it to done rather than asking for it to be deleted. The card number does not change, and you cannot set it. It is the short label that counts from 1 inside this project, and the cardId stays the value you pass here.')]
final readonly class CardUpdateTool implements FlagGatedToolInterface
{
     ...
     * @param string|null   $status          the slug of a new column on this board
