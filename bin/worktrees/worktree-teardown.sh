...
# Stop the sidecars first so their Traefik routes disappear before the code does
# — otherwise a route lingers and serves 502s. The compose file interpolates
# these even for `down`, so they must be set.
#
# --remove-orphans because the file above may be an older revision that never
# declared one of the running services; without it that container survives a
# teardown that reported success. -p scopes it to this worktree's own project.
WT_MAIN="$main" \
WT_ROOT="$root" \
WT_SLUG="$slug" \
WT_BASE_HOST="$project.dev.localhost" \
WT_APP_NETWORK="${project}_default" \
    docker compose -f "$compose_file" -p "${project}-wt-$slug" down --remove-orphans >/dev/null 2>&1 || true
