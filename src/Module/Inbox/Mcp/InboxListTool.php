 ...
 */
#[McpTool(name: self::NAME, description: 'List the items in the project inbox, newest first. Filter by state (open, answered, done, declined, withdrawn, obsolete), by askId to read the items of one ask, by sessionId to read the items of every ask a session made, by cardId or by documentId. An id that names nothing in this project matches nothing. Each row is a summary: itemId, number, kind, title, state, blocking, createdAt, updatedAt and closedAt. Call inbox_get with an itemId to read the body, the answer and the links. Paginated: pass page to walk further, and keep going while hasMore is true.')]
final readonly class InboxListTool implements FlagGatedToolInterface
{
     ...
     */
    public function __invoke(?string $state = null, ?string $askId = null, ?string $sessionId = null, ?string $cardId = null, ?string $documentId = null, int $page = 1, int $perPage = ListInboxItemsHandler::DEFAULT_PER_PAGE): array
    {
