...
The third check is the changelog entry, and it rides the branch that makes the change. It goes in its own fragment file, `changelog.d/<pull request number>.md`, never in `docs/CHANGELOG.md`. Two branches never write one fragment, so two entries never conflict. `just changelog` folds the fragments in, and the merge-queue holder runs it on `main` beside `just cs`. `working-with-prs` and `changelog.d/README.md` carry the format.
