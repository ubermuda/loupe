...
Tests run `fullyParallel: false` on 4 workers: parallel at the file level. A spec that changes a global flag (`/dev/e2e/feature-flag`, `/dev/billing-state`) or shares an account with another file, such as `ADMIN_EMAIL`, must go in a `workers: 1` project in `playwright.config.ts`. In `chromium` it races every file beside it. Each spec file that needs an authenticated session calls `createTest(credentials)` from `e2e/tests/fixtures.ts` with its own dedicated user. The worker fixture logs in as that user, registering and verifying it through Mailpit on the first run. Each file therefore has its own PHP session. That avoids CSRF token conflicts, and stops a mutation to one user (password re-hash, displayName change) invalidating another file's session. There is no shared setup phase: the fixture creates users lazily, on the file's first run.
